...
import useCurrentLocation from 'frontend/js/hooks/useCurrentLocation';
import useIntl from 'frontend/js/hooks/useIntl';
import classNames from 'classnames';
import SpinnerWheel from '../SpinnerWheel';
import useStyles from './useStyles';

const MyLocation = forwardRef(({ className, onLocation }, ref) => {
  const classes = useStyles();
  const intl = useIntl();
  const { execute, data, loading } = useCurrentLocation();
  ...
  return (
    <div className={classNames(className, classes.root)} ref={ref}>
      {loading ? (
        <SpinnerWheel color="black" className="is-in-button" size={14} style={{ marginRight: 4 }} />
      ) : (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 92.26 122.88">
