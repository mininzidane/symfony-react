...
function ZipFilterForm({ dispatch, section, query, refinements }) {
  ...
  const { loading, options } = useOptions(section, query, state.location);
  const { isBelowXs } = useBreakpoint();
  const { updateCustomerLocation } = useCustomerLocationContext();

  const isValidLocation = useMemo(
    () => Boolean(state.location?.lat && state.location?.lon && state.location?.zip),
    [state],
  );

  const isFormValid = useMemo(() => Boolean(isValidLocation && state.distance !== null), [state]);

  function refine(type = 'REFINE') {
    const distance = options.find((v) => v.value === state.distance);
    ...
    dispatch({
      ...
      payload: {
        ...
        section,
        values: {
          UOrigin: [state.location.lat, state.location.lon].join(','),
          distance: state.distance,
        },
        label: `${distance.label} - ${state.location.zip}`,
        meta: state.location,
      },
    });
  }
  ...
  function handleClick() {
    ...
    if (!state.location) {
      setError(intl.formatMessage({ id: 'searchResultsPage.filter.zip.enterZip' }));
    } else if (!(state.location?.lat && state.location?.lon && state.location?.zip)) {
      setError(intl.formatMessage({ id: 'searchResultsPage.filter.zip.invalidZip' }));
    ...
    LocalStorageService.set('abmSearchLocation', state.location);
  ...
  useEffect(() => {
    ...
    if (refinement && !refinement.label && isFormValid && options.length) {
      refine('FILL');
    }
  }, [refinements, isFormValid, options]);

  // Select 50mi by default
  useEffect(() => {
    if (state.distance === null && isValidLocation && options.length && !loading) {
      const matchesDefaultLabel = (label) => ['50 Miles', '50 mi'].some((v) => new RegExp(v, 'i').test(label));
      const defaultDistance = options.find((distance) => matchesDefaultLabel(distance.label));
    ...
    <div className={classes.root}>
      <div className={classes.grid}>
        <PlacesInputPlane
          ...
          className={classnames(classes.zipInput, error && !isValidLocation && 'is-error')}
          applyMask={(val) => {
        ...
        <SelectFilter
          ...
          error={Boolean(error && isValidLocation && !state.distance)}
        />
      </div>
